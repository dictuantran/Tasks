...
func GetCategories(username string) []types.CategoryCount {
	...
	stmt := `select 'UNCATEGORIZED' as name, count(1) 
				from task 
				where cat_id=0 
				union  
				select c.name, count(*)
				from category c
					left outer join task t on t.cat_id=c.id
					join status s on t.task_status_id=s.id
					where s.status != 'DELETED' and c.user_id=?
				group by name
				union
				select name, 0
				from category c
				where c.user_id=? 
						and name not in(
											...
											from task t
											join category c
											join status s on s.id = t.task_status_id and t.cat_id=c.id and s.status!='DELETED' and c.user_id=?
									)`
